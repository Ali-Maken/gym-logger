...
Offline-first Angular v22 PWA for logging gym sessions. Single user, single device, no backend, no HTTP calls.
...
- **One deep seam**: components import only the `Logbook` service (`src/app/core/logbook.ts`). Nothing outside `core/` may touch Dexie or `db.ts`.
- **Domain logic is pure functions** in `core/domain/` (hint rule, rotation, last-time selection, log formatting). Never put domain rules in components or templates.
- **Write-through order**: every mutation awaits the Dexie write FIRST, then updates the signal. This is the never-lose-data guarantee — do not reorder.
- **Signals only** at seams; no RxJS, no Dexie liveQuery.
...
- Deploy: `ng build --base-href /gym-logger/` → output into `docs/` (keep `docs/.nojekyll`). Pages already enabled: https://ali-maken.github.io/gym-logger/
- Hash routing (`withHashLocation()`), `navigator.storage.persist()` on startup.
