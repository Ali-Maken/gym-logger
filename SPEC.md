...
- **Angular v22** (current stable), standalone components, TypeScript strict mode, signals (no RxJS at app seams)
...
- **Hash routing** (`withHashLocation()`) — GitHub Pages has no rewrites; hash URLs make deep-link 404s impossible online and offline. No `404.html` trick.
...
src/app/
  core/
    logbook.ts        ← THE seam: signals + async methods; the only thing features may import
    db.ts             ← Dexie schema + seed (private to core)
    domain/           ← pure functions: hint rule, rotation, last-time selection, log formatting
  features/
    home/
    session/
      session.page.ts     ← thin container: route, Logbook wiring, layout selection, rest timer
...
- **Logbook** (`@Injectable`, root): the single deep module. Interface gist: `templates()`, `suggestion()`, `activeSession()`, `start(templateId)`, `completeSet(...)`, `setSetup(...)`, `finish()`, `lastTimeFor(exerciseId)`, `hintFor(entry)`, `exportJson()`, `importJson(payload)`. Components never touch Dexie.
- **Write path**: every mutation is `await db... put` **first**, then `signal.set(...)` — the never-lose-data guarantee.
- **`SessionView` seam**: the session container renders one of four dumb layout adapters, all consuming the same contract (session signals + callbacks `completeSet`, `step`, `choose`, `setSetup`, `skipTimer`). A future layout = one new adapter + a registry entry. Layouts contain **zero domain logic**.
...
- Build: `ng build --base-href /gym-logger/`, output copied into `docs/` (keep `.nojekyll`). ngsw registers relative to base href — no service-worker config edits needed.
- Updates: ngsw cache-busts `ngsw.json` on every open, defeating the Pages CDN's 10-min cache; new versions activate on next reload. Budget ~10 min propagation.
- Verification (each release): prod build served locally via `http-server` (SW never runs under `ng serve`) → DevTools offline mode → the `ngsw/state` debug page → on-device airplane-mode relaunch from the home-screen icon. Kill switch: delete `ngsw.json` from `docs/`.
- Full research notes: `.scratch/gym-logger/assets/research-angular-pwa-gh-pages.md`.

## Build order

Confirm each step works before the next.

1. **Scaffold + deploy the shell.** Angular v22 app, `@angular/pwa`, hash routing, built into `docs/`, replacing the placeholder. Verify installable + opens in airplane mode. (Pages side is already proven.)
